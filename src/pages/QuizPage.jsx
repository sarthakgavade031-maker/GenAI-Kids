...
export default function QuizPage() {
  ...
  const { lang } = useLang()
  const { markComplete, isComplete, addXp, XP_PER_QUIZ_CORRECT, XP_PER_QUIZ_COMPLETE } = useProgress()
  ...
  useEffect(() => {
    if (showResult && passed && !isComplete(age, topic)) {
      ...
      addXp(XP_PER_QUIZ_COMPLETE)
      setXpEarned(prev => prev + XP_PER_QUIZ_COMPLETE)
      setJustSaved(true)
    }
  }, [showResult])
  ...
  const handleAnswer = (idx) => {
    ...
    if (isCorrect) {
      ...
      setXpEarned(prev => prev + XP_PER_QUIZ_CORRECT)
    }
    ...
    return (
      <div className="app">
        ...
        <div className="quiz-container">
          ...
          {xpEarned > 0 && (
            <div style={{background:'rgba(167,139,250,0.15)', border:'1px solid #a78bfa', borderRadius:'12px', padding:'12px 20px', marginBottom:'20px', color:'#a78bfa', fontWeight:'700', textAlign:'center'}}>
              ⭐ +{xpEarned} XP Earned!
            </div>
          )}
