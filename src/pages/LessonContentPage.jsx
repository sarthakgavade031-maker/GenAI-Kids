...
export default function LessonContentPage() {
  ...
  const { lang } = useLang()
  const { markLessonRead, isLessonRead, XP_PER_LESSON_READ } = useProgress()
  ...
  useEffect(() => {
    if (isLast && !isLessonRead(age, topic)) {
      markLessonRead(age, topic)
      setShowXpToast(true)
    ...
    <div className="app">
      ...
      {showXpToast && (
        <div style={{
          ...
          fontWeight:800, fontSize:'0.95rem', boxShadow:'0 8px 24px rgba(167,139,250,0.4)',
          animation: 'fadeSlideIn 0.4s ease',
        }}>
          ⭐ +{XP_PER_LESSON_READ} XP — Lesson Read! 🎉
        </div>
      )}
